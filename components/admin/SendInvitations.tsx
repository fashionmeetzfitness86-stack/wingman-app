                ...
                {selectedEventId && filteredUsers.map(user => {
                    ...
                    return (
                        <div key={user.id} className="flex items-center gap-3 bg-gray-900/50 p-2 rounded-lg">
                            <input type="checkbox" checked={selectedUserIds.includes(user.id)} onChange={() => handleUserToggle(user.id)} className="h-5 w-5 rounded bg-gray-700 border-gray-600 text-gray-300 focus:ring-[#FFFFFF] flex-shrink-0" />
