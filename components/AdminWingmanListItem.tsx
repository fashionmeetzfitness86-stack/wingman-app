...
interface AdminWingmanListItemProps {
    wingman: Wingman;
    user: User;
    onEdit: (wingman: Wingman, user: User) => void;
    onDelete: (wingman: Wingman) => void;
    ...
    return (
        <div className={`relative rounded-lg overflow-hidden bg-transparent ${status !== 'active' ? 'opacity-60' : ''}`}>
            <div className="relative z-10 w-full bg-[#0F1014] border border-[#1C1D22] rounded-md p-4 flex items-center gap-4">
                <div onClick={() => onPreview(wingman)} className="flex-grow flex items-center gap-4 text-left cursor-pointer">
                    <img className="w-14 h-14 rounded-full object-cover flex-shrink-0" src={wingman.profilePhoto} alt={wingman.name} />
                    <div className="flex-grow grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                        <div>
                            <p className="font-bold text-white truncate">{wingman.name}</p>
                        ...
                        <div>
                            <p className="text-sm text-gray-300">{wingman.assignedVenueIds.length} Venues</p>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => onSuspend(user)} className="p-2 text-gray-400 hover:bg-white/5 rounded-md transition-colors" aria-label={status === 'suspended' ? `Unsuspend wingman ${wingman.name}` : `Suspend wingman ${wingman.name}`} title={status === 'suspended' ? 'Unsuspend' : 'Suspend'}>
                        {status === 'suspended' ? <UserPlusIcon className="w-5 h-5 text-green-400" /> : <UserMinusIcon className="w-5 h-5 text-yellow-400" />}
                    </button>
                    <button onClick={() => onEdit(wingman, user)} className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-md transition-colors" aria-label={`Edit wingman ${wingman.name}`} title="Edit">
                        <PencilSquareIcon className="w-5 h-5" />
                    </button>
                    <button onClick={() => onDelete(wingman)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-white/5 rounded-md transition-colors" aria-label={`Delete wingman ${wingman.name}`} title="Delete">
